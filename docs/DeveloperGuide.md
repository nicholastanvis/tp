...
1. Deleting a contact while all contacts are being shown
    ...
    1. Prerequisites: List all contacts using the `clist` command.
...
1. Dealing with missing/corrupted data files
    ...
    1. Note: If the app is run with a corrupted data file, the app will run itself with an empty data file. If it is run with a missing data file, the app will run with a data file containing sample contacts.

    1. To simulate a missing data file, locate the folder where you placed the jar file (where you run the app from). Remove the `data` folder.
    
    1. To simulate a corrupted data file, locate the folder mentioned above. Look for the `.json` file and simply add random characters at the end of the file.
